...
namespace signals
{
    ...
    public class Signal : BaseSignal
    {
		...
		public void RemoveAll()
		{
			...
			// Don't remove the first listener, as that will kill the base Listener instance so we won't be able to add to it anymore.
			if (listeners.Count() > 1)
			{
				for (int n = 1; n < listeners.Count(); n++)
				{
					Listener -= (Action)listeners[n];
				}
			}

			listeners = OnceListener.GetInvocationList();

			if (listeners.Count() > 1)
			{
				for (int n = 1; n < listeners.Count(); n++)
				{
					OnceListener -= (Action)listeners[n];
				}
			}
		}
    ...
    public class Signal<T> : BaseSignal
    {
		...
		public void RemoveAll()
		{
			...
			if (listeners.Count() > 1)
			{
				for (int n = 1; n < listeners.Count(); n++)
				{
					OnceListener -= (Action<T>)listeners[n];
				}
			}
		}
    ...
    public class Signal<T, U, V> : BaseSignal
    {
        ...
        public void Remove(Action<T, U, V> callback) 
		{ 
			Listener -= callback; 
		}
    ...
    public class Signal<T, U, V, W> : BaseSignal
    {
        ...
        public void Remove(Action<T, U, V, W> callback) 
		{ 
			Listener -= callback; 
		}
